...
namespace Repository.Dbo
{
    public abstract class BaseDbo
    {
        ...
        public static void Init(string databasePath, double busyTimeout = 30)
        {
            ...
            CreateTable<DeviseEntity>();
            CreateTable<CustomerEntity>();
            CreateTable<TransactionEntity>();
        }
        ...
        public static int Save(BaseEntity entity)
        {
            return Db.InsertOrReplace(entity);
        }

        public static void Save(IEnumerable<BaseEntity> entities)
        {
            lock (dbLock)
            {
                Db.RunInTransaction(() =>
